...
function onlineHandLayout(n: number, viewportW = 390) {
  ...
  if (n > 1) {
    ...
    if (need > avail) {
      ...
      if (cardW + (n - 1) * peek > avail) {
        peek = Math.max(16, Math.floor((avail - cardW) / (n - 1)))
        step = peek
      }
    } else {
      step = peek
  ...
  return {
    ...
    rotStep: n <= 3 ? 8.5 : n <= 5 ? 5.8 : n <= 7 ? 4.2 : n <= 10 ? 2.5 : n <= 13 ? 1.5 : 1.1,
    fanWidth: Math.round(n <= 1 ? cardW : Math.min(avail, cardW + (n - 1) * step)),
  }
}

function onlineHandFanY(offset: number, n: number) {
          ...
          >
            <div className="durak-hand-row">
              {view.you.map((c, i) => {
                ...
                const mid = (n - 1) / 2
                const offset = i - mid
                const fanY = onlineHandFanY(offset, n)
                const play = () => {
                  if (!legal) return
                  const now = Date.now()
